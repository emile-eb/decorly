...
export async function analyzeRoom(env, imageBytes) {
...
export async function describeMaterial(env, sampleBytes) {
    const client = createOpenAI(env);
...
async function trySharpDims(buf) {
    try {
...
async function generateReplace(env, inputImage, constraints) {
    ...
    const maskPath = constraints?.maskPath;
    ...
    if (maskPath) {
        try {
            ...
            if (providerName(env) === 'google') {
                ...
                if (!g)
                    throw new Error('google inpaint returned no image');
            ...
            const inDims = inDimsS || sizeOf(inputImage);
            const maskDims = maskDimsS || sizeOf(maskBytes);
    ...
    if (!b64)
        throw new Error('Image edits returned no data');
    const promptHash = createHash('sha256').update(finalPrompt).digest('hex');
    const promptPreview = finalPrompt.slice(0, 160);
    return { buffer: Buffer.from(b64, 'base64'), mode: 'edits', promptPreview, promptHash };
}
export async function generateRedesign(env, analysis, style, constraints) {
    // Build a richer prompt using domain-specific choices (defaults to interior)
    const flow = constraints?.flow || constraints?.mode || 'interior';
    if (flow === 'replace' && constraints?.__inputBytes) {
        return generateReplace(env, constraints.__inputBytes, constraints);
    }
    ...
    if (domain === 'interior') {
        ...
        const paletteColors = constraints?.palette_colors;
        choices = {
            ...choices,
    ...
    if (provider === 'google') {
        ...
        if (domain === 'interior') {
            ...
            const palLabel = choices.palette?.label || choices.palette || choices.palette_label;
            const palColors = choices.palette?.colors || choices.palette_colors;
            const roomType = choices.roomType || choices.room_type;
    ...
    const inputBytes = constraints?.__inputBytes;
    if (inputBytes && provider !== 'google') {
        try {
            const dims = sizeOf(inputBytes);
            const w = dims.width || 1024;
            const h = dims.height || 1024;
    ...
    let g = await imageGenerate(env, finalPrompt, hasInputForGenerate ? { imageBytes: constraints.__inputBytes, mime: constraints.__inputContentType || INPUT_IMAGE_MIME, jobId: constraints.__jobId } : { jobId: constraints.__jobId });
    if (!g)
...
async function postImageEdit(env, imageBytes, prompt, maskBytes, opts) {
    const provider = providerName(env);
    ...
    const imgFile = new File([imageBytes], 'input.jpg', { type: INPUT_IMAGE_MIME });
...
async function imageGenerate(env, prompt, opts) {
    const provider = providerName(env);
    if (provider === 'nanobanana') {
...
async function postNanoBananaEdit(env, imageBytes, prompt, maskBytes, opts) {
    if (!env.NB_API_URL || !env.NB_API_KEY)
        throw new Error('NanoBanana provider missing NB_API_URL or NB_API_KEY');
    if (typeof File === 'undefined')
        throw new Error('Runtime missing global File. Requires Node 18+ or a File polyfill.');
    const form = new FormData();
...
async function nanoBananaGenerate(env, prompt) {
    if (!env.NB_API_URL || !env.NB_API_KEY)
        throw new Error('NanoBanana provider missing NB_API_URL or NB_API_KEY');
