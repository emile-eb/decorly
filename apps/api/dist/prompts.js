export async function buildPrompt(env, input) {
    ...
    if (domain === 'interior') {
        ...
        if (override) {
            ...
            if (!styleText)
                throw new Error('Missing style for interior prompt');
            if (styleText)
                lines.push(`Target style: ${styleText}.`);
            const paletteLabelOv = choices.palette?.label || choices.palette || choices.palette_label;
            const paletteColorsOv = choices.palette?.colors || choices.palette_colors;
            if (paletteLabelOv)
                lines.push(`Use this color palette: ${paletteLabelOv}.`);
            if (paletteColorsOv?.length)
                lines.push(`Palette colors: ${paletteColorsOv.join(', ')}.`);
        }
        else {
            const styleText = choices.style === 'Custom' ? (choices.customStyle || '') : (choices.style || '');
            if (!styleText)
            ...
            const paletteLabel = choices.palette?.label || choices.palette || choices.palette_label;
            const paletteColors = choices.palette?.colors || choices.palette_colors;
            if (paletteLabel)
                lines.push(`Use this color palette: ${paletteLabel}.`);
            if (paletteColors?.length)
                lines.push(`Palette colors: ${paletteColors.join(', ')}.`);
        }
    }
    else if (domain === 'floor') {
        lines.push('You are an expert interior designer and flooring specialist.');
        const ft = choices.floorType?.trim();
        ...
        if (isRug) {
            ...
            if (choices.materialDesc)
                lines.push(`Material reference: ${choices.materialDesc}`);
            ...
            if (ft)
                lines.push(`Replace the existing floor with: ${ft}.`);
            if (choices.materialDesc)
                lines.push(`Material reference: ${choices.materialDesc}`);
        ...
        const areaType = choices.areaType || choices.area_type;
        const style = choices.style || choices.gardenStyle;
        const features = choices.features || [];
        if (areaType)
            lines.push(`Area type: ${areaType}.`);
        if (style)
            lines.push(`Garden style: ${style}.`);
        if (features.length)
            lines.push(`Include the following features: ${features.join(', ')}.`);
        ...
        const color = choices.color;
        if (color?.label || color?.hex) {
            const pieces = [];
            if (color.label)
                pieces.push(color.label);
            if (color.hex)
                pieces.push(`(${color.hex})`);
        ...
        const houseType = choices.houseType || choices.house_type;
        const style = choices.style || choices.homeStyle;
        const paletteLabel = choices.palette || choices.palette_label;
        const paletteColors = choices.palette_colors;
        if (houseType)
            lines.push(`House type: ${houseType}.`);
        if (style)
            lines.push(`Exterior style: ${style}.`);
        if (paletteLabel)
            lines.push(`Color palette: ${paletteLabel}.`);
        if (paletteColors?.length)
            lines.push(`Palette colors: ${paletteColors.join(', ')}.`);
        ...
        const focusAreas = choices.areas || choices.focus_areas || [];
        const keepList = choices.keep || choices.keep_list || [];
        if (focusAreas.length)
            lines.push(`Focus areas: ${focusAreas.join(', ')}.`);
        if (keepList.length)
            lines.push(`Keep list (never remove): ${keepList.join(', ')}.`);
    }
