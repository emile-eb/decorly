...
async function processOne() {
    ...
    else if (raw.claim_next_job) {
        job = raw.claim_next_job;
    }
    if (!job || !job.id)
        return false;
    const id = job?.id;
    try {
        ...
        const inputBucket = job.input_bucket || 'room_inputs';
        const inputPath = job.input_image_path;
        ...
        if (flow === 'interior' && env.PASS_THROUGH_INTERIOR) {
            try {
        ...
        if (flow === 'replace') {
            ...
            try {
                if (Array.isArray(constraints.marks) && constraints.canvas?.width && constraints.canvas?.height) {
                    ...
                    for (const m of constraints.marks) {
                    ...
                    // Encode PNG
                    const maskBuffer = PNG.sync.write(png);
        ...
        const { buffer: outputBuffer, mode: generationMode, promptPreview, promptHash } = await generateRedesign(env, analysis, job.style || 'modern', constraints);
        // A) After decode
        ...
        if (decodedBytes < 50000)
            throw new Error(`decoded image too small: ${decodedBytes} bytes`);
        const outputBucket = job.output_bucket || 'room_outputs';
